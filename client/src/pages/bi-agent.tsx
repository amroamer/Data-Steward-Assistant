            ...
            {expanded && (
              <div style={{ padding: "12px 14px", background: "#0A162899" }}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                  <button onClick={() => setTestStatus(prev => ({ ...prev, [tcId]: "pass" }))} style={{ padding: "4px 12px", borderRadius: 6, border: `1px solid ${status === "pass" ? "#2E7D32" : "#2A4A6E"}`, background: status === "pass" ? "rgba(46,125,50,0.3)" : "transparent", color: status === "pass" ? "#81C784" : "#5A8AB8", cursor: "pointer", fontSize: 11, fontWeight: 600 }} data-testid={`btn-pass-${tcId}`}>✓ Pass</button>
                  <button onClick={() => setTestStatus(prev => ({ ...prev, [tcId]: "fail" }))} style={{ padding: "4px 12px", borderRadius: 6, border: `1px solid ${status === "fail" ? "#B71C1C" : "#2A4A6E"}`, background: status === "fail" ? "rgba(183,28,28,0.3)" : "transparent", color: status === "fail" ? "#EF9A9A" : "#5A8AB8", cursor: "pointer", fontSize: 11, fontWeight: 600 }} data-testid={`btn-fail-${tcId}`}>✕ Fail</button>
                </div>
              </div>
            )}
          </div>
