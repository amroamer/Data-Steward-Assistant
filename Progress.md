...
| Date | Change |
|---|---|
| 2026-03-11 | **NDMO Data Classification — 4-level upgrade** — aligned the entire classification pipeline with the official SDAIA NDMO National Data Governance Interim Regulations (Section 4.3). Replaced the old 5-level framework (Top Secret/Secret/Confidential/Restricted/Public) with the correct 4-level NDMO framework: Top Secret (TS) = High Impact, Secret (S) = Medium Impact, Confidential (C) = Low Impact with sub-levels A/B/C, Public (P) = No Impact. System prompt now includes full NDMO Section 4.3 text with impact categories, ZATCA examples per level, and classification rules. Markdown table expanded to 9 columns: Field Name, Classification Level, Classification Code, Confidential Sub-Level, Impact Level, Impact Category, Justification, PII Under PDPL, Recommended Controls. `ResultRow` and `ANALYSIS_COLUMNS` in `result-store.ts` updated with all new fields + header mappings. Excel `data_classification` sheet applies color coding to Classification Level cells: dark navy (#1A1A2E) for Top Secret, red (#C0392B) for Secret, orange (#E67E22) for Confidential, green (#27AE60) for Public. Chat summary shows 4-level breakdown with emojis: 🔴 TS, 🟠 S, 🟡 C, 🟢 P plus overall dataset classification. "Restricted" removed from all prompts and UI |
| 2026-03-11 | **result.xlsx in-app preview** — added a blue 👁 Preview button next to the Download button on the result.xlsx card in the Outputs panel; clicking it builds the workbook in-memory using new `buildResultWorkbook()` (extracted from `generateResultExcel`), converts to a `File` object, and opens the existing ExcelPreview full-screen modal with sheet tabs, frozen header row, and horizontal scroll; no download is triggered; closes with click-outside or X button |
...
| 2026-03-11 | **Business Intelligence Agent Page** — new `/bi-agent` page with 5 feature tabs: Sharing Eligibility, Dashboard Designer, Report Tester, Test Case Generator, Dashboard Tester; 5 backend endpoints (`POST /api/bi/sharing-eligibility`, `/api/bi/dashboard-designer`, `/api/bi/report-tester`, `/api/bi/test-case-generator`, `/api/bi/dashboard-tester`); `bi-store.ts` Excel library generates cumulative `bi_agent_report.xlsx` with 8 sheets (sharing_eligibility, dashboard_design, dax_measures, kpis, report_test_results, test_cases, test_summary, dashboard_test_cases); shared file upload across tabs; AbortController stop generation; recharts donut charts for coverage; pass/fail tracking on test cases; in-memory session threads; EN+AR bilingual; ZATCA brand colors; sidebar "BI Agent" link with Brain icon; route registered in App.tsx |
