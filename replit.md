...
- **Core Features**:
    ...
    - **Business Intelligence Agent**: Standalone `/bi-agent` page with 5 tabs (Sharing Eligibility, Dashboard Designer, Report Tester, Test Case Generator, Dashboard Tester). 5 backend endpoints under `/api/bi/*`. `bi-store.ts` generates cumulative `bi_agent_report.xlsx` with 8 sheets. Shared file upload, AbortController stop, recharts donut charts, pass/fail test tracking, EN+AR, ZATCA branding. Accessed via agent-mode-tabs (next to Nudge Agent tab).
