...
- **Core Features**:
    ...
    - **Data Sharing Eligibility Tool**: Standalone `/sharing-eligibility` page; `POST /api/sharing-eligibility` backend calls Claude with NDMO Section 4.3 + Section 6 rules; drag-and-drop file upload; field detection; AI classification per field; overall verdict banner; 3 recipient cards; anonymization tip; recommended actions; expandable field table; Excel download.
    - **Business Intelligence Agent**: Inline `agentMode="bi"` within chat.tsx (same pattern as Nudge). 5 sub-tabs: Sharing Eligibility, Dashboard Designer, Report Tester, Test Case Generator, Dashboard Tester. 5 backend endpoints under `/api/bi/*`. `bi-store.ts` generates cumulative `bi_agent_report.xlsx` with 8 sheets. 5 inline result components (BiSharingResult, BiDashboardResult, BiReportResult, BiTestCaseResult, BiDashboardTestResult). Pass/fail test tracking, EN+AR, ZATCA branding.
    - **5-Agent Navigation Bar**: Below top header — tabs for "Data Management" (Database icon), "Analytical Model" (Layers icon), "Insights Agent" (Brain icon), "Nudge Agent" (Target icon, purple), "BI Agent" (BarChart3 icon, ZATCA blue). Active tab = navy bg, white text. Controls which feature cards, pills, and input panel are shown.
